...
import { ShieldAlert, Globe, Users, Loader2, FileText, ExternalLink, AlertTriangle, CheckCircle, Info } from 'lucide-react';
             ...
             {!loading && mode === 'internet' && internetResult && (
               <div className="space-y-6">
                 {internetResult.sources.length > 0 ? (
                    <div className="p-4 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl">
                      <h4 className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2 mb-3">
                        <Globe size={18} className="text-blue-500" /> Top Web Sources Found
                      </h4>
                      <ul className="space-y-2">
                        {internetResult.sources.slice(0, 3).map((source, idx) => (
                          <li key={idx} className="bg-white dark:bg-slate-800 p-2 rounded border border-slate-100 dark:border-slate-700 flex items-center justify-between text-sm hover:shadow-sm transition-shadow">
                             <div className="flex items-center gap-2 overflow-hidden">
                               <div className="w-5 h-5 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center flex-shrink-0 text-xs font-bold text-blue-600 dark:text-blue-400">
                                 {idx + 1}
                               </div>
                    ...
                    <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800 rounded-xl text-green-800 dark:text-green-300 flex items-center gap-3">
                      <CheckCircle size={24} />
                      <div>
                        <p className="font-bold">No matching web sources found.</p>
                        <p className="text-xs">The text appears unique compared to online results.</p>
                      </div>
                    </div>
                 )}

                 <div className="prose prose-slate dark:prose-invert max-w-none prose-sm">
                   <h4 className="text-slate-800 dark:text-slate-200 font-bold mb-2">Detailed AI Analysis</h4>
