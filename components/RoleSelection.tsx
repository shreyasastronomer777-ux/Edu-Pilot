    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#050505] flex items-center justify-center p-8 transition-colors duration-500">
      <div className="max-w-4xl w-full">
        ...
        <div className="grid md:grid-cols-2 gap-10 max-w-3xl mx-auto">
          {roles.map((role, idx) => (
            <button 
              ...
              className="bg-white dark:bg-white/[0.03] p-12 rounded-[3.5rem] border border-slate-200 dark:border-white/10 hover:border-indigo-500 dark:hover:border-indigo-500 hover:shadow-3xl transition-all group text-left relative overflow-hidden flex flex-col h-full animate-in fade-in slide-in-from-bottom-8 duration-700 shadow-sm"
            >
              <div className="absolute top-0 right-0 p-10 opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="bg-indigo-600 text-white p-3 rounded-full shadow-xl">
                  <ArrowRight size={24} />
                </div>
              </div>
              
              <div className={`w-20 h-20 bg-${role.color}-100 dark:bg-white/5 text-${role.color}-600 dark:text-white rounded-[1.8rem] flex items-center justify-center mb-10 group-hover:scale-110 transition-transform duration-500`}>
                <role.icon size={42} />
              </div>
              
              <h3 className="text-4xl font-black text-slate-900 dark:text-white mb-4 tracking-tight uppercase leading-none">{role.label}</h3>
              <p className="text-slate-500 dark:text-slate-400 leading-relaxed text-base font-medium flex-1 opacity-80 group-hover:opacity-100 transition-opacity">
                {role.desc}
              </p>
            </button>
          ))}
        ...
        <div className="mt-24 text-center animate-in fade-in duration-1000 delay-500">
            <p className="text-[11px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-[0.4em]">
